#include "trans_with_behaviour.h"
...
int main(int argc,char**argv){
	...
	char* parafile = "iparameter_behaviour.txt"; //file name
	...
	if(myid ==0){
		/*
			generate randomseeds send to slave
		*/
		cout<<"hello"<<endl;
		currentInfo[0] = SEED_NUM;  //infect
		currentInfo[1] = 0;      //dead
	...
	for(int i=0;i<n;i++){
		...
		for(int t=0;t<T;t++){       // transmit time from 0 to t
			...
			if(myid!=0){
	            ...
	            while(iter!=myarea->infected_now.end()){
		        ...
		        while(iter_id != myarea->just_infected.end()){  //区分，也就是处理本区域的，然后存储其它区域的
...
void Initial_subarea(){
    if(myid!=0){
        ...
        vector<int>& temp = myarea->first_infected;
        for(int i = 0;i < temp.size();i ++){
        	myarea->Individuals[temp[i]]->GetDisease(myarea,0);
        }//for
    }  
}
void send_data_to_master(vector<pair<int,double> >& v,int size){
    ...
    MPI_Send(&slave_size,1,MPI_INT,0,98,MPI_COMM_WORLD);//感染人数
    MPI_Send(infectious_id1,slave_size,MPI_INT,0,97,MPI_COMM_WORLD);//感染id
    MPI_Send(infectious_time1,slave_size,MPI_DOUBLE,0,96,MPI_COMM_WORLD);//感染时间
    MPI_Send(&size,1,MPI_INT,0,50,MPI_COMM_WORLD);//预防接种人数
    MPI_Send(prevention_id1,size,MPI_INT,0,49,MPI_COMM_WORLD);//接种id
