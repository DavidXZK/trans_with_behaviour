...
class SubArea
{
	public:
		...
		vector<int> first_infected;//初始感染者列表
...
void Traversal_singal(unordered_map<int,int>& umap,int& infect,int& dead){   //记录每次的信息
	...
	while(it != umap.end()){
		int pid = it->first;
		unordered_map<int,individual*>::iterator it_temp = myarea->individuals.find(pid);
		if(it_temp != myarea->individuals.end()){
			if(it_temp->second->disease_state == 1){
				infect ++;
			}
			if(it_temp->second->disease_state == 3){
				dead ++;
			}
		}//if
		it ++;
...
void Traversal(individual* in,int&infect,int&dead){
	Traversal_singal(in->home,infect,dead);
	Traversal_singal(in->school,infect,dead);
	Traversal_singal(in->friends,infect,dead);
	Traversal_singal(in->work,infect,dead);
	Traversal_singal(in->commute,infect,dead);
	Traversal_singal(in->community,infect,dead);
}
void CalInfect(SubArea* apt,Individual* inv,double re_infectious_rate,double current_time,double period)
{
